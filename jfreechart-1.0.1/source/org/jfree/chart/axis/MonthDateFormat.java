 ...
 * Contributor(s):   -;
 *
 * $Id: MonthDateFormat.java,v 1.1.2.3 2005/10/25 20:37:34 mungady Exp $
 *
 * Changes:
 * --------
...
public class MonthDateFormat extends DateFormat {
    ...
    public MonthDateFormat(TimeZone zone, Locale locale, int chars, 
        ...
        this(
            zone, locale, chars, new boolean[] {showYearForJan, false, false, 
    ...
    public MonthDateFormat(TimeZone zone, Locale locale, int chars, 
    	...
    	for (int i = 0; i < 12; i++) {
    		if (chars > 0) {
                months[i] = monthsFromLocale[i].substring(
                    0, Math.min(chars, monthsFromLocale[i].length())
                );
    		}
    		else {
