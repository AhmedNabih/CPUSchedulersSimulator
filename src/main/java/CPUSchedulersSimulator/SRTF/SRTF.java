...
public class SRTF {
				 ...
				 if(Procces[i].arrivalTime <= startTime)
			       {
			        ...
			        {
			        	Procces[i].TurnAroundTime++;//If process has arrived and it has not already completed execution its TT is incremented by 1
			        	if(c!=oldC) // not the same process 
			        		{
			        		Procces[i].TurnAroundTime++; //Context Switching 
		            		}

			            if(i != c)//If the process has not been currently assigned the CPU and has arrived its WT is incremented by 1
			            	Procces[i].waitingTime++;
			        }
			        else if(i == c)//This is a special case in which the process has been assigned CPU and has completed its execution
			 ...
			 if((startTime-1-miniArrivalTime) != 0)
		      {
		       ...
		       System.out.print((startTime-1) + "--P" + (c+1));
		      if((startTime-1-miniArrivalTime) == total_time-1)//All the process names have been printed now we have to print the time at which execution ends
		       {
	...
	public static void PrintAverage(SRTF Procces[],int n)
	{
		...
		for(int i=0;i<n;i++)
		{
			...
			System.out.println((i+1)+"\t"+"\t"+Procces[i].arrivalTime+"\t"+"\t"+Procces[i].TurnAroundTime+"\t"+"\t"+Procces[i].waitingTime);
		}
